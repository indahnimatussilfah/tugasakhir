<nav class="p-4 rounded-xl bg-sky-600 shadow-md">
    <ul class="flex justify-end gap-10 pr-32">
        ...
        <li>
            <a href="{{ route('gis.index') }}" class="text-white hover:text-yellow-300 font-semibold">Layanan Kesehatan Terdekat</a>
        </li>
...
@auth
    @if (auth()->user()->role === 'masyarakat')
        <li class="relative">
    <div class="flex items-center space-x-2 text-white font-semibold cursor-pointer peer">
        <span>{{ auth()->user()->name }}</span>
        <img class="w-8 h-8 rounded-full" src="{{ asset('assets/img/undraw_profile.svg') }}" alt="User profile">
    </div>

    <!-- Dropdown menu -->
    <div
        class="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg z-50 hidden peer-hover:flex hover:flex flex-col pt-2"
    >
        <a href="{{ route('profile.index') }}" class="px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
            Profile
        </a>
        <a href="{{ route('settings.index') }}" class="px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
            Settings
        </a>
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit" class="text-left w-full px-4 py-2 text-sm text-gray-700 hover:bg-gray-100">
                Logout
            </button>
        </form>
    </div>
